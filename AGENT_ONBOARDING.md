...
- **The task isn't done until GitHub Copilot's review is addressed.** Copilot auto-reviews PRs here. After pushing, read its comments (`gh pr view <n> --comments`; inline threads via `gh api repos/{owner}/{repo}/pulls/<n>/comments`) and either fix each actionable point and push, or reply explaining why it doesn't apply. It re-reviews on each push, so repeat until nothing actionable remains.
