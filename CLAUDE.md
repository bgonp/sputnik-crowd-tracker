...
6. **Wait for GitHub Copilot's review and address it.** Copilot auto-reviews a PR once when it's opened — give it a moment, then read its comments (`gh pr view <n> --comments`, and `gh api repos/{owner}/{repo}/pulls/<n>/comments` for inline review threads). The task is **not** done while Copilot has open feedback — for each actionable comment, either fix it and push, or reply explaining why it doesn't apply; ignore only clearly spurious ones, and say so. Copilot does **not** re-review automatically when you push fixes — explicitly re-request it (`gh api --method POST repos/{owner}/{repo}/pulls/<n>/requested_reviewers -f 'reviewers[]=Copilot'`), then wait for the new review. Repeat until it has nothing actionable left.
